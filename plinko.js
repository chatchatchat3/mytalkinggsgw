function createPlinkoGame(opts) {
  const {
    Engine,
    Render,
    Runner,
  ...
  const background = opts.background || "#00e000";
  ...
  function makeWalls() {
    const wallOpts = { isStatic: true, render: { fillStyle: "#888" } };
    return [
      Bodies.rectangle(WIDTH / 2, HEIGHT + WALL / 2, WIDTH, WALL, wallOpts),
      Bodies.rectangle(-WALL / 2, HEIGHT / 2, WALL, HEIGHT, wallOpts),
      Bodies.rectangle(WIDTH + WALL / 2, HEIGHT / 2, WALL, HEIGHT, wallOpts),
    ];
  }
  ...
  function spawnFloat(x, y, text, color) {
    floats.push({ x, y, text, color: color || "#fff", born: performance.now() });
  }
      ...
      if (!horse.scored && t.position.y > SCORE_LINE_Y) {
        ...
        if (speed < REST_SPEED) {
          ...
          if (zone) {
            ...
            const res = onLand(zone.value);
            if (res) {
              const label = typeof res === "string" ? { text: res } : res;
              spawnFloat(t.position.x, t.position.y, label.text, label.color);
            }
          }
        }
      }
  ...
  Events.on(render, "afterRender", () => {
    ...
    for (const z of zones) {
      ctx.fillStyle = z.value === 0 ? "#c0392b" : z.value >= 5 ? "#000" : "#888";
      ctx.fillText("x" + z.value, z.cx, HEIGHT - 22);
    }

    const FLOAT_DUR = 1200;
    const FLOAT_RISE = 60;
    ...
    for (let i = floats.length - 1; i >= 0; i--) {
      ...
      const p = age / FLOAT_DUR;
      const y = f.y - p * FLOAT_RISE;
      ctx.globalAlpha = 1 - p;
